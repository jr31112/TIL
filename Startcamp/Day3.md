...
* `form`태그는  `action`속성으로 해당 `form`이 전송될 url을 지정해야한다.
...
2. 정보 받아서 활용하기
   ...
   def pong():
       ...
       return render_template('pong.html', say=text)
   ```
   ...
   * `request.args`는 일종의 `dictionary`이고, `key` 는input에 정의한 name이고 사용자가 입력한 값은 value다